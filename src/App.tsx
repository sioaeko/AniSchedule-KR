...
import { Clock3, CalendarDays, Info, Tv, Globe2 } from 'lucide-react';
...
function scoreCandidate(item: TMDBSearchResult['results'][number], title: string, titleJa: string) {
...
function App() {
  ...
  const shouldExcludeAnime = useCallback((anime: AnimeSchedule) => {
    ...
    const hasExcludedKeyword = excludedKeywords.some((keyword) =>
      titleLower.includes(keyword.toLowerCase()) || japaneseTitleLower.includes(keyword.toLowerCase())
    );
    ...
    const hasKodomoDemographic = anime.demographics?.some(
      (demo) => demo.name.toLowerCase() === 'kids' || demo.name.toLowerCase() === 'kodomo'
    );
  ...
  const fetchKoreanTitle = useCallback(async (title: string, japaneseTitle: string) => {
    ...
    for (let attempt = 0; attempt < 2; attempt++) {
      try {
        const [withTitle, withJapanese] = await Promise.all([doSearch(title), doSearch(japaneseTitle)]);
        const pool = [...withTitle, ...withJapanese];
        if (pool.length === 0) return title;
              ...
              <article key={`${show.mal_id}-${show.broadcast?.time ?? 'na'}`} className="overflow-hidden rounded-xl border border-slate-800 bg-slate-900">
                <div className="relative h-44 bg-slate-800">
                  {show.images.jpg.image_url ? (
                    <img src={show.images.jpg.image_url} alt={show.title_korean ?? show.title} className="h-full w-full object-cover" loading="lazy" />
                  ) : (
                ...
                <div className="space-y-3 p-4">
                  ...
                  <div className="flex flex-wrap gap-2 text-xs text-slate-200">
                    <span className="inline-flex items-center gap-1 rounded-md bg-slate-800 px-2 py-1"><Clock3 className="h-3.5 w-3.5" />{formatTime(show.broadcast?.time)}</span>
                    {!!show.broadcast?.day && <span className="inline-flex items-center gap-1 rounded-md bg-slate-800 px-2 py-1"><Tv className="h-3.5 w-3.5" />{show.broadcast.day}</span>}
                  </div>
                  <div className="flex flex-wrap gap-1.5">
                    {show.genres?.slice(0, 4).map((genre) => (
                      <span key={`${show.mal_id}-${genre.name}`} className="rounded-full border border-slate-700 px-2 py-0.5 text-xs text-slate-300">{genre.name}</span>
                    ))}
                  </div>
                </div>
              </article>
